...
- [x] **Remove Layer 3b CPU fallback (dead code, 0% recovery rate)** (#93) — Production evidence (0/1,038 CPU recovery rate across 2026-05-07 downstream-consumer run) showed the CPU fallback never recovered any inference; `cpuPredictorFactory` property and all wiring removed; Layer 3b branch deleted from `predictWindow`; Layer 3a failure path now explicitly logs + rethrows; `logRecoveredIOSurface`, `logCPUFallbackFailed`, `extractCPUErrorFields` methods deleted; `cpu_fallback_failed` JSONL category retired; two new Layer 3a stub tests added (`testANERetrySucceedsAfterReload`, `testANERetryFailsLogsErrorRow`); ADR 021 second addendum
- [x] **`Float16` x86_64 universal-binary build fix** (#115) — `Float16` is unavailable on x86_64 macOS (Swift compile error), breaking Release-config archive builds. Replaced `Float16` pointer-rebind in `CoreMLModelIO.readEmbeddingTensor` with a pure-Swift IEEE 754 half-precision decoder (`float32FromFloat16Bits`) that uses `UInt16.self` on all architectures (Strategy B: no `#if arch(arm64)` branch). New `Float16Codec.swift` helper (internal, no CoreML dependency). 288-test suite: known-pattern test for all 11 spec-required bit patterns + arm64 oracle test sweeping all 65 536 FP16 bit patterns against native `Float(Float16(bitPattern:))`.
...
- [x] **LSM compaction observability** (#110) — Four structured `os.Logger` info-level events in `performFlush()`: `[Compact] started` (input_segments, input_bytes, trigger), `[Compact] finished` (adds output_segments, output_bytes, elapsed_ms), `[Compact] cancelled` (processed_segments/total, elapsed_ms; requires single pre-write `Task.checkCancellation()` checkpoint at step-9 boundary), `[Compact] failed` (input_segments, elapsed_ms, error). Trigger inferred from `targetLevel`: `cascade` when > 0, `forced` when == 0. All numeric/enum values `privacy: .public`; error description `privacy: .auto`. Reuses existing `indexerLogger`; no new subsystem/category. ADR 004 section (h) added.
- [x] **Memory-bounded cascade compaction** (#112, ADR 027) — Tiled O(B × k) SGEMM in `KMeans.assignAll()` replaces the O(m × k) = O(m^1.5) dense scores matrix; B = 4 096 caps peak tile at ~330 MB at reproducer scale vs ~128 GB before fix. Eliminated `allRows: [[Float]]` intermediate in `performFlush()` (saves ~876 MB of per-token Swift Array overhead at reproducer scale). Added `autoreleasepool { BucketRecord(...) }` around per-bucket codec calls to drain transient Foundation Data objects promptly. Peak phys_footprint delta during 3-input-segment 600K-embedding cascade test: **342 MB** (< 4× ledger soft target of 1 171 MB, << 32 GB hard gate). Opt-in `CompactionMemoryRegression` benchmark suite (`SWITCHCRAFT_COMPACT_REGRESSION=1`). Deferred: in-memory ledger restructuring (`[Int64: [[Float]]] → [Int64: [Float]]`) — not required to satisfy the hard gate; follow-up issue. **Done, #137/ADR 036** (the shipped shape is `[Int64: [LedgerToken]]`, a per-token materialized-or-bucket-ref sum type, not the flat-`[Float]` shape originally sketched here — see below).
...
- [x] **FTS title field for title-weight BM25 boosting** (#108, ADR 026) — `DocumentRecord` gains `title: String?`; SQLite `document` table gains `title TEXT`; `document_fts` redefined with `fts5(title, body, ...)` (title first); V0→V1 schema migration auto-runs on first `open()` of an existing database (`PRAGMA user_version` gate); `HybridConfig.ftsTitleWeight: Float` (default 3.0) configures per-column BM25 weight; `InMemoryStorage` applies title-weighted token-overlap scoring; `StorageConformance` gains `runFullTextTitleWeighting` test; migration test verifies V0→V1 path; ADRs 009, 025, 026 updated. Callers that omit `title` get byte-identical behaviour.
- [x] **Query-token surface-form length filter for short proper-noun precision** (#117, ADR 028) — `SearchConfig.queryMinSurfaceFormLength: Int = 0` (default preserves Witchcraft parity); `Embedder` protocol gains `encodeQuery(_:minSurfaceFormLength:)` with a default no-op forwarding to `encode`; `T5CoreMLEmbedder` overrides it with real filtering after `SlidingWindow.merge()` — tokens whose decoded SentencePiece surface form has `count <= threshold` are dropped; `T5CoreMLEmbedder.encode()` and `encodeQuery()` share a private `_encodeImpl` to avoid re-entrancy deadlock; `SwitchcraftStore.search()` calls `encodeQuery` so the filter applies on the query path only. Fixes noise-floor precision collapse for single-word queries with short subword decompositions (e.g. `"bartleby"` → `["bar", "t", "le", "by"]`) at corpus scale. Default `0` leaves NFCorpus + 33-fact corpus unaffected. New `NoiseFloorPrecisionTests` asset-gated suite: baseline (< 0.4 precision@5 without filter), fix (≥ 0.6 precision@5 with threshold=2), top-1 preserved (35-doc scale).
- [x] **Orphan chunk detection and recovery** (#120, ADR 029) — `Store.add()` dedup branch now checks for orphaned chunks (chunk row exists but no bucket assignments) via a new `SwitchcraftStorage.chunkHasBucketAssignments(_:)` protocol method (default `true` — opt-out for external conformers). Orphans are automatically recovered by calling `indexer.add()` with freshly-computed embeddings (R1). Within-process pending chunks tracked in `Store.pendingChunkIDs: Set<Int64>` to guard against double-buffering (R2). `flushAndClearPending()` private helper replaces all direct `indexer.flush()` calls. New public `Store.findOrphanedChunks() -> [OrphanedChunkInfo]` API enumerates all chunks lacking bucket assignments, with `owningDocuments` field for recovery guidance. `SwitchcraftStorage` gains `allChunks()` (default `[]`) for the bulk scan. All conformers (`InMemoryStorage`, `SQLiteStorage`) implement both new methods. `StorageConformance` extended with `runAllChunks` and `runChunkHasBucketAssignments` scenarios. New `OrphanTests` suite: orphan-recovery-via-add (R4), pending-dedup-not-double-buffered (R2), findOrphanedChunks correctness (R9). Dedup-hit benchmark confirms < 100 µs/call overhead (release, InMemoryStorage). NFCorpus and 33-fact corpus unaffected.
- [x] **Mid-operation compaction ledger divergence fix** (#123, ADR 030) — Self-recovery block added in `Indexer.performFlush()` at the `m != total` check. When divergence is detected, finds "surprise gens" (gens not in `mergedGens` whose chunk-ID range overlaps `[minChunkID..maxChunkID]`), extends `mergedGens`, widens the range, recomputes `total`, rebuilds the ledger row arrays, and re-checks `m == total`. If no surprise gens exist the original `ledgerOutOfSync` error is still thrown (true invariant violation). `mergedGens`, `m`, and `sortedChunkIDs` changed from `let` to `var`; no protocol changes, no public API changes. Root cause: ADR 029 orphan recovery calls `indexer.add()` between flush-queue entry and `performFlush()` execution, causing the cascade walk's chunk-ID range to sweep in rows from higher-level gens not included in `mergedGens`. Three new tests in `IndexerConflictRecoveryTests`: mid-operation self-recovery (no throw), `findOrphanedChunks` idempotence after simulated interruption, performance guard (2× baseline ceiling for real workloads). ADR 030 added.
- [x] **WAL checkpoint contract: `CheckpointResult`, flush-first `walCheckpoint()`, `shutdown()` partial logging, WAL-bounds regression test** (#127, ADR 033) — New `CheckpointResult` public enum (`.complete` / `.partial(framesRemaining:)`) in `SwitchcraftCore`. `SwitchcraftStorage.walCheckpoint()` protocol requirement updated to return `CheckpointResult`; default extension returns `.complete` so non-SQLite backends compile unchanged. `SQLiteWriterActor.walCheckpoint()` replaces `conn.execute()` with `conn.prepare()` + step-based read of `(busy, log)` columns from `PRAGMA wal_checkpoint(TRUNCATE)`. `SQLiteStorage.walCheckpoint()` propagates `CheckpointResult` to caller. `SwitchcraftStore.walCheckpoint()` flushes indexer first (matching `shutdown()` semantics), returns `CheckpointResult`. `SwitchcraftStore.shutdown()` unpacks result and logs via `os_log` at `.error` on `.partial`. WAL-bounds regression test (`testWALBoundsAcrossShutdownCycles`) verifies WAL stays bounded across N shutdown/reopen cycles and that explicit `walCheckpoint()` preserves durability.
- [x] **Partial-orphan dedup predicate fix** (#130, ADR 029 amended) — R3 guard in `Store.add()` changed from `chunkHasBucketAssignments() == true` (presence) to `chunkBucketRefCount() >= existing.counts.reduce(0, +)` (completeness). Partial-indexing anomalies (`0 < refs < expected`) are now recovered alongside full orphans (`refs == 0`). `SwitchcraftStorage.chunkHasBucketAssignments` replaced by `chunkBucketRefCount(_ chunkID: Int64) -> Int` (default `Int.max`). `InMemoryStorage.indexedChunkIDsCache` restructured from `Set<Int64>?` to `[Int64: Int]?` counting pairs per chunk. SQLite backends accumulate total pair count instead of short-circuiting on first hit. `Indexer.removeFromLedger(_ chunkID:)` added — clears stale rehydrated ledger rows before `indexer.add()` in the recovery path, preventing wrong `tokenOffset` assignments from ledger appending. ADR 029 §b table, §c recovery contract, §d rationale, and New Storage Protocol Methods section amended. `StorageConformance.runChunkHasBucketAssignments` renamed to `runChunkBucketRefCount` with count assertions. New `OrphanTests.partialOrphanRecoveredOnAdd` test (R6). NFCorpus and 33-fact corpus unaffected.
- [x] **Partial-orphan repair `ledgerOutOfSync` fix** (#132, ADR 029 amended) — `removeFromLedger()` in `Indexer` now accumulates removed row counts in `removedFromLedgerCount: Int`. `performFlush()`'s cascade walk subtracts this from `pendingCount` before initialising `total`, eliminating the false `m < total` drift when a partial orphan's rehydrated rows are removed and replaced. Reset on flush success (step 12) and in `clearIndex()`. `OrphanTests.partialOrphanRecoveredOnAdd` updated to use `numEmbeddings=1` with a process-restart simulation (close + reopen over same `InMemoryStorage`). Two new tests: `partialOrphanBatchRecoveryNoLedgerSync` (AC6: batch recovery of 3 partial orphans, no throw) and `partialOrphanRecoveryIdempotent` (AC7: two recovery passes on a partially-repaired corpus, monotone partial-count reduction). ADR 029 amended with root-cause analysis, fix description, and ADR 024/030 cross-references.
- [x] **`vacuum()` — remove abandoned chunks and reclaim disk** (#134, ADR 029 amended) — new public `SwitchcraftStore.vacuum(maxBatch: Int? = nil) async throws -> VacuumResult`, the complementary operation to `findOrphanedChunks()`: targets chunks with **no** owning document (vs. `findOrphanedChunks()`'s "has a document but is under-indexed"). Consumer-driven batch looping via `VacuumResult.remainingCandidates`; idempotent across repeated bounded calls. Cascade-deletes emptied buckets/generations and corrects surviving generations' `numEmbeddings` via a new pure `VacuumPlanBuilder` (range-pruned against the batch's chunk-id span) that produces a data-only `VacuumPlan`, executed atomically by three new `SwitchcraftStorage` methods (`documentHashes()`, `applyVacuumPlan(_:)`, `freeListByteCount()`, all with safe-default extensions) implemented in both `InMemoryStorage` and `SQLiteStorage`/`SQLiteWriterActor`. The chunk-row delete is guarded (`NOT EXISTS` in SQLite; hash-membership check in-memory) against a concurrent re-add racing vacuum's write phase. Ledger consistency is vacuum's own responsibility via a new targeted `Indexer.removeAbandonedFromLedger(_:)` (not a full re-init, which would reintroduce an ADR-030-class race) — deliberately does not touch `removedFromLedgerCount` since vacuum performs a true delete, not a clear-and-refill. `approximateDiskReclaimed` is measured as the `PRAGMA freelist_count × page_size` delta around the write phase (explicitly does not imply file-size shrinkage; documented via doc comment and the SQLite-only `vacuumFreelistAndFileSizeContract` test). New `VacuumResult` public type in `Sources/Switchcraft/`. `StorageConformance` gains `runDocumentHashes`/`runApplyVacuumPlan`/`runFreeListByteCount`. New `VacuumTests` suite (parameterized over `InMemoryStorage` + in-memory `SQLiteStorage`): basic removal correctness, clean-store idempotency, cascade + no-`ledgerOutOfSync` regression, batch/loop parity vs. a single unbounded call, partial-orphan-with-owning-document is never removed, mixed-corpus no-cross-contamination with `findOrphanedChunks()`, SQLite-only freelist/file-size pragma contract, and a ≥1,200-chunk sustained-workload test (file-backed SQLite, bounded WAL across looped batches). `findOrphanedChunks()` and existing `OrphanTests` unchanged. NFCorpus and 33-fact corpus unaffected (asset-gated, unchanged pass/skip state). **Amended, #142/ADR 037**: `removeAbandonedFromLedger(_:)` (since renamed to `applyVacuumLedgerUpdates(abandonedChunkIDs:remaps:)` — see below) did not account for #137's lazy `.bucketRef` ledger tokens — compacting a bucket shared between an abandoned and a still-live chunk left the live chunk's `.bucketRef` pointing at its pre-compaction offset, either throwing spuriously or silently resolving to a different pair's embedding. See below (#142) for the fix.
- [x] **Ledger snapshot fast path — skip full rehydration walk on clean-shutdown startups** (#136, ADR 034) — new persisted single-slot ledger snapshot lets `Indexer.init` skip the `O(all embeddings)` rehydration walk (10–30+ min at ~11 M-embedding scale) on clean-shutdown restarts. Three new `SwitchcraftStorage` methods (`saveLedgerSnapshot`/`loadLedgerSnapshot`/`clearLedgerSnapshot`, all with safe no-op default extensions — opt-out precedent, not correctness-critical) implemented in `InMemoryStorage` (stored `LedgerSnapshotRecord?`) and SQLite (single-row `ledger_snapshot` table, UPSERT on `id=1`, created via `CREATE TABLE IF NOT EXISTS` — no `user_version` bump). New `LedgerSnapshotRecord` (Records.swift) + `LedgerSnapshotCodec`/`LedgerSnapshotFingerprint` (new file): payload encodes the full `[Int64:[[Float]]]` ledger bit-for-bit (full-precision floats, deterministic chunkID order), fingerprint = `chunkCount` + derived `maxChunkID`/`totalEmbeddings`/`maxGenerationID`/`generationCount` from `generations()` (no new storage query). `Indexer.init` tries `loadFromSnapshotIfValid()` first: fingerprint-match + clean decode → adopt ledger/dims, `recoveredConflictCount=0`, clear-on-load (invalidate-on-load); any miss (absent/stale/corrupt) falls through to the **unchanged** `rehydrateThrowError`/`rehydrateAutoRecover` walk. Write points: best-effort `writeLedgerSnapshot()` at the end of `flush()`'s leader success path, and unconditional public `persistSnapshot()` (flush-leader-gated) from `SwitchcraftStore.shutdown()` before `walCheckpoint()` (closes the Research-flagged idle/read-only-session gap). Write-time consistency guard (total ledger rows == `sum(gen.numEmbeddings)`) refuses to persist a ledger diverged from storage out-of-band, preserving the #132 partial-orphan fallback. Renamed test-only `Indexer.ledgerSnapshot` → `ledgerContents`; added public `Indexer.didUseSnapshotFastPath`. `StorageConformance.runSnapshotLifecycle` (both backends). New `LedgerSnapshotCodecTests`, `IndexerSnapshotFastPathTests`, `StoreSnapshotShutdownTests` (file-backed shutdown→reopen reproducer). ADR 024/033 semantics unchanged; NFCorpus + 33-fact corpus unaffected.
- [x] **Parallel bucket search + query-token deduplication** (#140, ADR 035) — `SearchEngine.search`'s default path (`searchOptimized`) now: (1) deduplicates bit-exact-equal query-token embedding rows before centroid selection/scoring, re-weighting each unique row's contribution by duplicate count at the final mean-aggregation step (ADR 028's no-op proof, applied in the collapse direction); (2) decodes the deduplicated candidate bucket set (`center + residual` reconstruction — the dominant per-search cost per issue #140's profiling) across `TaskGroup` child tasks that touch no actor-isolated state, using `vDSP_vadd` instead of the prior scalar per-dimension loop, with results written into order-preserving slots so output stays byte-identical to the sequential baseline. The pre-#140 implementation is retained verbatim as `searchLegacy`, reachable only via the new benchmark-only `SearchConfig.legacySequentialSearch` flag. ADR 006(f)'s determinism claim is amended (not weakened) to describe the `TaskGroup` mechanism; dedup's own guarantee is documented separately (tolerance-bounded scores for duplicate-token queries, bit-exact otherwise) since `v * count` and repeated float addition aren't bit-identical. `tPrime` early-termination (the issue's option 2) was evaluated and explicitly not implemented — it would change candidate selection itself, not just its speed, carrying NDCG-parity risk the other two options don't have. New `SearchDeterminismTests` (repeated-search byte-identical, `searchLegacy`/`searchOptimized` equivalence incl. duplicate-token queries, concurrent-overlapping-search reentrancy) and `SearchLatencyBenchmarkTests` (opt-in via `SWITCHCRAFT_SEARCH_LATENCY_BENCHMARK=1`: ratio benchmark against a ~557K-embedding/768-dim/two-generation synthetic fixture measured 22x speedup, Release, well above the required 3x; 100-query sustained-workload check measured 1.02x first-quartile-median-to-last-quartile-p95 ratio, well under the 2.0x degradation tolerance). Both NFCorpus quality gates (CoreML `[0.31,0.33]`, Metal `[0.31,0.34]`) remain required (optimisation touches encoder-agnostic code) — asset-gated, unchanged pass/skip state in this environment. Surface-form pre-filter / IDF-filter investigation documented in ADR 035 (no default-value change).
- [x] **Lazy embedding materialization — bucket-ref ledger tokens, reconstruct on demand at compaction time** (#137, ADR 036) — `Indexer.ledger`'s inner value restructured from `[[Float]]` to `[LedgerToken]` (`.materialized([Float])` for not-yet-flushed L0 rows; `.bucketRef(genID:bucketID:pairOffset:)` for anything already flushed). Both rehydration paths populate bucket-refs directly from bucket metadata via a cheap `O(pairs)` init-time integrity walk (`checkedBucketCenter`/`checkedResidualsSize`: center non-empty, indices decode, residuals blob size — no residual-*value* decode) instead of the old `O(pairs×dims)` `center + dequantized residual` reconstruction; `rehydrateAutoRecover` confines the one remaining residual-value decode to buckets in generations the winner/loser conflict resolution (ADR 024) actually prunes. `performFlush()`'s three original ledger-read consumers (ADR 027's `allFlat` build, ADR 030's self-recovery rebuild, the sqrt-sample training loop) collapse into one batched materialization pass (`collectAndMaterialize`/`Indexer.BucketDecodeCache`) that decodes each referenced bucket at most once regardless of how many sampled tokens land in it, then transitions just-flushed tokens to bucket-refs pointing at the new generation. Two new `Indexer.Error` cases: `rehydrationBucketCorrupt` (reused, cheaper checks) and new `bucketRefUnresolvable(chunkID:bucketID:cause:)` (hard compaction failure on a runtime materialization error the init-time walk didn't catch — no silent-skip, since a partial training set would silently corrupt k-means output). `LedgerSnapshotCodec` (ADR 034) payload bumped to a v2 tag-per-token format (magic+version header, per-token materialized/bucket-ref tag) so `writeLedgerSnapshot()` stays a pure in-memory encode of whatever the ledger already holds rather than force-materializing every bucket-ref on the flush hot path; a v1 (pre-#137) snapshot is safely rejected and falls back to full rehydration. ADR 034's "snapshot is more faithful, full-precision" claim corrected (amended in place) — both paths are now consistently Q4-lossy for any previously-flushed row. `ledgerContents()` (test-only accessor) is now `async throws`, resolving all bucket-refs via the same batched helper. New `LazyLedgerMaterializationTests` (10K-chunk functional reproducer: rehydrate is O(generations) with no decode, later materialization batches to one storage read per generation and matches a golden reference computed independently from raw bucket bytes — Requirements 9/10), opt-in `LazyLedgerBenchmarkTests` (`SWITCHCRAFT_LAZY_LEDGER_BENCHMARK=1`; 100K-chunk/1M-embedding/768-dim sustained-workload benchmark; memory asserted as a ≥30× ratio in both build configs, time asserted as a ≥50× ratio in Debug but as an absolute ≤1.0s ceiling in Release since `-O` vectorization asymmetry between the arithmetic-heavy deleted eager loop and the bookkeeping-heavy lazy path shrinks the Release-mode ratio without indicating a regression — see ADR 036 §8), and a v1-snapshot-fallback integration test. NFCorpus and 33-fact corpus benchmarks re-run and unaffected (asset-gated in this environment; unchanged pass/skip state).
- [x] **Vacuum shared-bucket compaction invalidates live chunks' lazy `.bucketRef`s** (#142, ADR 037) — fix for a data-correctness bug at the intersection of #134 (vacuum) and #137 (lazy ledger, ADR 036): compacting a bucket shared between an abandoned chunk and a still-live chunk shifted the live chunk's pair offset without updating its ledger `.bucketRef`, either throwing spurious `bucketRefUnresolvable` or silently resolving to a different pair's embedding. `VacuumPlanBuilder.buildPlan` now accumulates a `BucketRefRemap` (chunkID, tokenOffset, genID, bucketID, newPairOffset) for every surviving pair in the same per-bucket compaction loop that produces the re-encoded blob — no extra decode pass, no `SwitchcraftStorage` protocol change. `Indexer.removeAbandonedFromLedger(_:)` renamed to `applyVacuumLedgerUpdates(abandonedChunkIDs:remaps:)`, applying each remap via the same `ledger[chunkID]?[tokenOffset] = .bucketRef(...)` pattern `performFlush()` already uses (ADR 036 §4); blind overwrite, no defensive pre-check, since `vacuum()`'s documented contract already excludes concurrent same-process writers. `VacuumTests.swift` gains three regression tests: a hand-planted shared-bucket out-of-range reproducer, an adversarial in-range silent-wrong-value test (asserts the live chunk's resolved embedding equals its own golden value and explicitly differs from the value it would have silently resolved to pre-fix), and a 20+-cycle sustained shared-bucket workload test that forces a cascade after every vacuum call — necessary because neither vacuum's own counters nor `search()` observe a stale ref, only a subsequent cascade's `resolveBucketRefRow` call can. All three tests verified to fail (reproducing the exact issue symptom) against the pre-fix code and pass against the fix. `findOrphanedChunks()`/R1 recovery and `bucketRefUnresolvable`'s error semantics unchanged. NFCorpus and 33-fact corpus unaffected (asset-gated, unchanged skip state; no scoring path touched).
- [x] **Shutdown-time snapshot encode: instrumentation, bulk-buffer rewrite, yield points, progress hook** (#144, ADR 038) — fix for a downstream consumer's reported 5–20+s (occasionally unbounded) shutdown delay, all inside `SwitchcraftStore.shutdown()`'s unconditional-every-shutdown `persistSnapshot()` call (ADR 034 §4). `shutdown()` now logs three `.info` lines (`switchcraft-adapter-shutdown: wal_checkpoint took Xs` / `persistSnapshot took Xs (encode Xs, write Xs, bytes=N, chunks=N)` / `total Xs`) via a new `Sendable` `PersistSnapshotStats` struct returned up from `Indexer.writeLedgerSnapshot()`/`persistSnapshot()` rather than logged from `SwitchcraftCore` directly. `LedgerSnapshotCodec.encode(_:dims:)` first gained an exact-size `reserveCapacity` pre-pass (own commit, clean before/after baseline), then was rewritten around a preallocated `[UInt8]` buffer with indexed writes instead of per-byte `Data.append` (hundreds of millions of individual append calls at production scale, ~11.1M embeddings, was the diagnosed dominant cost — not raw bytes-per-row, so an incremental/delta-snapshot redesign was evaluated and explicitly rejected in favor of this direct fix, see ADR 038 §3). `encode()` is now `async` with a periodic `await Task.yield()` every 512 chunks (safe because every caller holds the `flushInProgress` leader gate for the call's duration — now an explicit documented precondition) and an optional `onProgress: (@Sendable (SnapshotProgress) async -> Void)?` callback invoked at the same checkpoints, threaded through `writeLedgerSnapshot(onProgress:)` → `persistSnapshot(onProgress:)` → `SwitchcraftStore.shutdown(onSnapshotProgress:)` (per-call closure, not a stored property/`AsyncStream` — ADR 032's rejected-Option-A concerns don't apply since there is exactly one call site). New `Sources/SwitchcraftCore/Indexer/SnapshotProgress.swift` (`SnapshotProgress`, `PersistSnapshotStats`). Wire format, write-time consistency guard, fingerprint mechanics, and shutdown-write-before-`walCheckpoint()` ordering (ADR 034/036) are unchanged. New tests: byte-for-byte golden-output wire-format regression (independent hand-built reference, guards against the rewrite silently mis-indexing bytes), `onProgress` checkpoint-cadence test, a deterministic (non-wall-clock-racing) concurrent-task-interleaving reproducer for the yield points, and an opt-in (`SWITCHCRAFT_SHUTDOWN_LATENCY_BENCHMARK=1`, Release-gated) corpus-scale benchmark asserting `shutdown()` completes in <2s at ~7,300-chunk/~11.1M-embedding/768-dim scale (measured ~0.4s Release; Debug measured ~223s due to unoptimized `inout`/exclusivity-check overhead, not user-representative, hence Release-gated matching `IndexerSnapshotSpeedupTests`/`LazyLedgerBenchmarkTests` precedent) — fixture built by writing a synthetic pure-`.bucketRef` ledger snapshot directly to storage rather than synthesizing real Q4 residual data (infeasible at this scale, ~34GB transient). All 12 existing `LedgerSnapshotCodecTests.swift` round-trip call sites updated with `await`.
- [x] **Wall-clock threshold assertions: absolute floors, `ContinuousClock`, multi-sample medians** (#146, ADR 039) — fix for `performanceSmoke`/`searchWriteStallRegressionTest` intermittently failing full-suite `swift test` (debug) runs under host-load/Swift-Testing intra-process-parallelism contention (found during #144's Validate stage). `IndexerTests.performanceSmoke`: `Date()` → `ContinuousClock`; inaccurate "ADR 012, ~1.35× CI-runner factor" comment (borrowed from a different, release-mode-only suite's rejected calibration) replaced with fresh 14-run measurement evidence — 10s/5s limits left **unchanged**, since re-measurement confirmed `#97`'s `vDSP_maxvi` fix headroom still holds (median 3.916s, max 6.513s, no drift from `#137`'s later `performFlush()` rewrite). `SQLiteStorageConcurrencyTests.measureMedian`: default `iterations` 1→5 (was a mislabeled single sample, not an actual median). `searchWriteStallRegressionTest`: `Date()` → `ContinuousClock`; ceiling changed from bare `tIsolated * 1.5` to `max(tIsolated * 1.5, tIsolated + 0.030)` — an absolute floor (mirroring `IndexerConflictRecoveryTests`' existing ratio-OR-floor precedent) so small-baseline scheduling jitter isn't a large relative overshoot; 30ms slack derived from and validated against the same 14-run measurement set (floor term gated in every run; ratio term never bound). Both fixes confirmed via 14 consecutive full-suite runs on a sustained heavy-load host (0 failures for either target test). Diagnostic `print()` statements added to both tests (`[PerfSmoke]`/`[StallRegression]`), matching the existing `[PerfTest]` precedent in `IndexerConflictRecoveryTests`. Test-file-scoped only — no `SwitchcraftStorage` protocol change, no production-code change. Related finding (not fixed, out of scope for #146): `ConcurrencyTests` R1–R4 recorded `.searchTimedOut` in 2/14 of the same runs — same failure class, recommended as a follow-up issue in ADR 039.
- [x] **Per-stage search timing instrumentation** (#148, ADR 040) — observability fix for A downstream consumer report ("Switchcraft search consistently exceeds 5s Release deadline"), whose Research stage found no per-stage timing existed to attribute the overrun to flush/cascade, embed, or `searchHybrid` retrieval. `SearchEngine.searchHybrid()` now returns `HybridSearchResult` (`hits: [HybridHit]`, `timing: HybridSearchTiming` — `vectorDuration`/`bm25Duration`/`mergeDuration`, all non-optional, measured with paired `ContinuousClock.now` reads at each stage boundary since every boundary crosses an actor hop). `SwitchcraftStore.search()` now returns `SearchResult` (`hits: [HybridHit]`, `timing: SearchTiming` — `flushDuration`/`cascadeCompactionOccurred`/`cascadeCompactionDuration`/`embedDuration` plus the three `HybridSearchTiming` fields, all `Duration?` except the `Bool` occurred flag, since early-throw paths genuinely predate later stages). `cascadeCompactionDuration` equals `flushDuration` whenever the existing `CompactionEvent.trigger == .cascade` (ADR 032) — no `Indexer`/`CompactionEvent` changes needed. `flushAndClearPending()` is now `@discardableResult ... -> (event: CompactionEvent?, duration: Duration)`; its other 5 call sites are unaffected. `SwitchcraftStoreError.searchTimedOut` gained a `partialTiming: SearchTiming` field (arity change, source-breaking for external pattern-matchers) populated from a call-local `var timing` in `search()` — deliberately not a shared actor property, since `SwitchcraftStore` permits reentrancy and a shared "last call" var would race across concurrent `search()` calls. `SQLiteStorage.translateIfInterrupt(_:)`'s direct `searchTimedOut` throw (a call site outside `SwitchcraftStore.swift` the Plan stage didn't anticipate) reports an empty `SearchTiming()`, since that storage-layer interrupt path has no visibility into the store's accumulated timing. ~79 in-package call sites (mostly tests) mechanically updated with `.hits`. New `Tests/SwitchcraftTests/SearchTimingTests.swift`: independent-observability tests (slow BM25/vector/embed via new `DelayInjectingStorage`/`DelayedEmbedder` test doubles, millisecond-scale delays only — no hardcoded seconds-scale sleeps), cascade-detection true/false via `IndexerConfig.testing(l0Capacity:)`, and partial-timing-on-timeout. `SearchTimeoutTests.swift`'s two `searchTimedOut` pattern-match sites updated; one gained `partialTiming` assertions. No retrieval, flush, or cascade behavior changed — purely additive instrumentation.
- [x] **Two-phase centroid scan — scan + fetch, eliminating unnecessary `indices`/`residuals` materialization** (#151, ADR 041) — fix for a materialization-waste finding downstream of a downstream consumer report: `SearchEngine.searchLegacy`/`searchOptimized`'s centroid scan called `storage.buckets(forGeneration:)` for every generation but used only `center` and `residuals.count` per bucket — on the reporter's live index (72,773 buckets, 11.6M token embeddings) this copied 797 MiB out of SQLite per search, ~95% (`indices`+`residuals` payload bytes) discarded unread. New `BucketScanRecord` (`id`, `generationID`, `center`, `residualByteCount`) and two new required `SwitchcraftStorage` protocol methods: `scanBuckets(forGeneration:)` (scan phase, `id ASC` order preserved — load-bearing for `cblas_sgemm` summation determinism) and `buckets(ids:)` (fetch phase, unspecified order, for exactly the buckets selection picked). `SQLiteReaderActor`/`SQLiteStorage` (`.inMemory` + `.fileBacked` modes) implement `scanBuckets` via SQL `length(residuals)` (blob length header only, never the payload — the change is meaningless if it still fetches the bytes internally, per the issue's own explicit caveat) and `buckets(ids:)` via a batched `WHERE id IN (...)` (chunks of 500, under `SQLITE_LIMIT_VARIABLE_NUMBER`). `InMemoryStorage` implements both directly (no I/O cost to save; `buckets(ids:)` is a linear scan, no new index — see ADR 041 §3 for why). Both `searchLegacy` and `searchOptimized` (oracle-coupled via `SearchDeterminismTests`) migrated together: `selected: [BucketRecord]` becomes `selectedIDs: [Int64]` during the scan loop (same dedup logic), one `buckets(ids:)` call after the loop fetches full records, reconstructed in `selectedIDs` order (not fetch-return order) so `decodeSelectedBuckets` (ADR 035, unchanged) stays byte-identical. Missing ids during fetch (a narrow scan/fetch race, e.g. concurrent vacuum) are silently dropped, matching the pre-existing generations()/buckets(forGeneration:) race tolerance — no new `SearchEngine.Error` case. `StorageConformance` gains `runBucketScanLifecycle`/`runBucketsByIDs`/`runBucketsByIDsExceedsBatchSize` (id-ASC order, missing-id tolerance, multi-generation id sets, >500-id batch boundary), covering all three existing storage configurations for free. New `InMemoryStorageBucketsByIDsRegressionTests` (buckets(ids:) correctness across `deleteGeneration`/`replaceGeneration`/`applyVacuumPlan`) and a parameterized `SQLiteStorageConformanceTests` case (`scanBuckets` `residualByteCount` matches full-fetch `residuals.count`, both `.inMemory` and `.fileBacked`). New always-on (not env-var-gated — the fixture is ~4,000 small buckets, sub-0.5s) `BucketScanBytesReadTests`: two on-disk-SQLite generations, equal bucket count, 200x per-bucket residual-size difference, asserts `scanBuckets` elapsed time (9-trial minimum) stays flat (observed ~1.4-1.8x) while `buckets(forGeneration:)` visibly scales (observed ~10-13x, asserted >3x as a methodology control). `SearchDeterminismTests` (all 4 tests) and `SearchLatencyBenchmarkTests` (both Debug and Release) re-run and pass unchanged — the ratio benchmark's speedup incidentally improved (5.0x Debug / 22.5x Release vs. the required ≥3x) as a side effect, though that benchmark was already passing before this issue and isn't the primary evidence (`BucketScanBytesReadTests` is). No selection/scoring/decode algorithm change; NFCorpus and 33-fact corpus unaffected (no scoring path touched). **Validate-stage fix**: the original 300-bucket/7-trial-median benchmark flaked ~37% of the time under full-suite contention (operations ran at 100-600μs, far below the "tens of milliseconds" floor ADR 039 requires for ratio-based wall-clock thresholds); root-caused and fixed by raising the fixture to 2,000 buckets/9-trial minimum (ADR 039 update, issue #151 commit `8c93196`).
...
downstream consumers — they can build against
...
conformance — consumer-selected at init time. The downstream consumer (and
any other consumer) keeps working with the CoreML
