...
- [x] **Port ggml's T5 inference to Swift + Metal** (gives us roughly Witchcraft's embedder size + speed + power; Metal kernels themselves don't reach ANE today but the `Embedder` seam preserves a future CoreML-FP16 variant if/when one becomes viable) — see "Port ggml's T5 inference to Swift + Metal" below
  ...
  - [x] **#65** — NFCorpus NDCG@10 gate: ∈ [0.31, 0.33] with `T5MetalEmbedder` active; cross-stack tolerance retune (ADR 010(h) gains a Metal-vs-Witchcraft sub-section with `2 × empirical maxAbs`, alongside the existing CoreML-vs-Witchcraft ±0.025 entry — both pairs documented per their distinct precision axes per ADR 014); informational `T5MetalEmbedder.encode` and end-to-end search p95 reporting via `EmbedderPerformanceTests`; Plan tick on the parent line above.
