...
- [ ] **Port ggml's T5 inference to Swift + Metal** (gives us roughly Witchcraft's embedder size + speed + power; Metal kernels themselves don't reach ANE today but the `Embedder` seam preserves a future CoreML-FP16 variant if/when one becomes viable) — see "Port ggml's T5 inference to Swift + Metal" below. **All seven kernel/orchestration sub-issues (#58–#64) merged + #65 test/ADR scaffolding landed; tick lands once the operator runs the NDCG@10 gate green and retunes the cross-stack tolerance per ADR 010(h).**
  ...
  - [ ] **#65** — NFCorpus NDCG@10 gate: ∈ [0.31, 0.33] with `T5MetalEmbedder` active; cross-stack tolerance retune (ADR 010(h) ratchets back toward ±0.01 once the precision pair becomes Q4_K-vs-Q4_K); Plan tick on the parent line above. **Test/ADR scaffolding merged (PR #73): asset-gated `NFCorpusMetalBenchmarkTests`, `CrossStackEmbeddingParityMetalTests`, `EmbedderPerformanceTests`; ADR 010(h) Metal sub-section landed with placeholder `0.01` tolerance. Tick lands when an operator runs the suites locally with `SWITCHCRAFT_XTR_GGUF` + regenerated `reference_embeddings.{bin,json}`, confirms NDCG@10 ∈ [0.31, 0.33], and retunes the test constant + ADR sub-section to `2 × observed maxAbs`.**
