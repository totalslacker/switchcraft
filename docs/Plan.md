...
- [x] **Per-search wall-time deadline and cooperative task cancellation** (#83, ADR 019) — `SearchConfig.searchDeadline` (default 5 s), per-call `deadline: Duration?` override on `SwitchcraftStore.search()`, pre-phase deadline checkpoints, `sqlite3_progress_handler` with remaining-budget enforcement, `Task.checkCancellation()` at key storage checkpoints, `SwitchcraftStoreError.searchTimedOut(elapsed:)`
