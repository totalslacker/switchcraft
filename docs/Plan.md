...
- [ ] **Port ggml's T5 inference to Swift + Metal** (gives us roughly Witchcraft's embedder size + speed + power; Metal kernels themselves don't reach ANE today but the `Embedder` seam preserves a future CoreML-FP16 variant if/when one becomes viable) — see "Port ggml's T5 inference to Swift + Metal" below
  ...
  - [ ] **#59** — `SwitchcraftMetal` library target + GGUF reader for Q4_K weights + ADR 010(j) (asset format and `SWITCHCRAFT_XTR_GGUF` env-var gating). Lifts `MetalContext` visibility to `@_spi(SwitchcraftMetal) public`; reuses #51's foundation.
  - [ ] **#60** — Q4_K matmul kernel: port `kernel_mul_mat_q4_K_f32` + `kernel_dequantize_q4_K` from `ggml-metal.metal` to MSL; ≥0.99999 cosine vs C reference on per-kernel inputs.
  - [ ] **#61** — RMSNorm kernel: port `kernel_rms_norm_f32` to MSL; FP32 throughout per ADR 003 + ADR 014(b); ≥0.99999 cosine on per-token inputs.
  - [ ] **#62** — Softmax kernel + relative-position bias: port `kernel_soft_max_f32`; implement T5 `_relative_position_bucket` (formula in catalogue) and gather per-head bias pre-softmax.
  - [ ] **#63** — Element-wise kernels: residual add (`kernel_add_f32`), gated-GELU (`gelu_new(wi_0(x)) * wi_1(x)` — three matmuls per FFN block, not two), L2 normalisation (FP32 — `sum(x²)` overflows FP16 per ADR 014(b)).
  - [ ] **#64** — `T5MetalEmbedder` orchestration: encoder forward pass dispatch order, `MTLBuffer` lifecycle, `Embedder` protocol conformance (ADR 009); ADR 016 (per-op precision matrix promoted to normative); ≥0.99999 cosine vs PyTorch FP32 reference.
  - [ ] **#65** — NFCorpus NDCG@10 gate: ∈ [0.31, 0.33] with `T5MetalEmbedder` active; cross-stack tolerance retune (ADR 010(h) ratchets back toward ±0.01 once the precision pair becomes Q4_K-vs-Q4_K); Plan tick on the parent line above.
...
- **NDCG@10**: ∈ [0.31, 0.33] on NFCorpus when `T5MetalEmbedder`
