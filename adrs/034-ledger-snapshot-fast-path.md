...
retrieval results would silently drift. Notably the snapshot stores *full-precision*
floats, so the fast-path ledger is exact, whereas the full-walk reconstruction is
Q4-lossy — the fast path is if anything *more* faithful to the originally-added
embeddings.
