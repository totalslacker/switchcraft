...
Root Cause 1 — the missing `.serialized` trait — was not addressed by the deletion alone. The two remaining tests (`livenessTest`, `searchWriteStallRegressionTest`) each seed a 5,000-document SQLite database at startup. When run concurrently (the default), their I/O contaminates each other's timing measurements. Specifically, `searchWriteStallRegressionTest`'s isolated baseline (`tIsolated`) is measured while sibling tests are doing heavy I/O, yielding anomalously low values (2–4 ms instead of the expected 7–10 ms) that make the 1.5× ceiling impossibly tight.
...
The actor-serialization regression that `performanceAssertionTest` was meant to catch — concurrent search holding the storage actor and blocking indexer writes — is covered by `searchWriteStallRegressionTest` and `livenessTest`, both of which remain in the suite and run on every CI push.

`searchWriteStallRegressionTest` verifies that 50 sequential writes are not stalled by a concurrently-running FTS scan, using a 1.5× overhead ceiling over an isolated write baseline. This directly tests the stall scenario from the original downstream-consumer bug (1,178/6,511 chunks indexed when a search froze the indexer).
...
Together, `livenessTest` and `searchWriteStallRegressionTest` provide the coverage pair described in the issue spec.
...
- The speedup metric (`tBoth < tRead + tWrite × 0.7`) is no longer actively measured anywhere. This is acceptable: the split architecture is validated structurally by ADR 019, the stall regression is what matters to users, and `searchWriteStallRegressionTest` + `livenessTest` together catch regressions in that behavior.
