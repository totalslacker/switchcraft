...
//
// Test support helpers for the SwitchcraftMetal kernel suites.
//
// **Duplicate of `Tests/SwitchcraftTests/Metal/MetalKernelTestSupport.swift`**
// per the issue #60 plan §"Tests + perf helpers duplicated, not extracted"
// and the Q3 resolution comment. Refactor trigger: if duplicated four-plus
// times by the time #61–#63 land, extract a shared `MetalTestSupport`
// target both test targets depend on. Until then any change to one copy
